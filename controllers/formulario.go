...
type FormularioController struct {
	beego.Controller
}

// URLMapping ...
func (c *FormularioController) URLMapping() {
...
// @Param	body		body 	models.Formulario	true		"body for Formulario content"
// @Success 201 {int} models.Formulario
// @Failure 403 body is empty
// @router / [post]
func (c *FormularioController) Post() {
	var v models.Formulario
...
// @Success 200 {object} models.Formulario
// @Failure 403 :id is empty
// @router /:id [get]
func (c *FormularioController) GetOne() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	v, err := models.GetFormularioById(id)
	if err != nil {
		logs.Error(err)
...
// @Failure 403
// @router / [get]
func (c *FormularioController) GetAll() {
	var fields []string
	var sortby []string
	var order []string
	...
	l, err := models.GetAllFormulario(query, fields, sortby, order, offset, limit)
	if err != nil {
		logs.Error(err)
...
// @router /:id [put]
func (c *FormularioController) Put() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	v := models.Formulario{Id: id}
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		v.FechaModificacion = time_bogota.TiempoBogotaFormato()
		if err := models.UpdateFormularioById(&v); err == nil {
...
// @Failure 403 id is empty
// @router /:id [delete]
func (c *FormularioController) Delete() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	if err := models.DeleteFormulario(id); err == nil {
