...
// @Success 201 {int} models.Plantilla
...
func (c *PlantillaController) Post() {
	...
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		if _, err := models.AddPlantilla(&v); err == nil {
...
func (c *PlantillaController) GetOne() {
	...
	v, err := models.GetPlantillaById(id)
	if err != nil {
		logs.Error(err)
...
func (c *PlantillaController) GetAll() {
	...
	l, err := models.GetAllPlantilla(query, fields, sortby, order, offset, limit)
	if err != nil {
		logs.Error(err)
...
func (c *PlantillaController) Delete() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	if err := models.DeletePlantilla(id); err == nil {
